...
import com.ly.rshypoc.face.FaceInfoCache;
import com.ly.rshypoc.face.FaceView;
import com.ly.rshypoc.face.FaceViewCache;
import com.ly.rshypoc.util.CollectionUtils;
import com.ly.rshypoc.util.DefaultHttpObserver;
 ...
 */
public class XyCallPresenterBase implements XyCallContractBase.Presenter {
    private static final String TAG = "XyCallPresenter";
    private XyCallContractBase.View mCallView;
    ...
    TextView tvNumber;
    TextView iv_host_num;

    public XyCallPresenterBase(XyCallContractBase.View callView) {
        this.mCallView = callView;
        callView.setPresenter(this);
        faceInfoCache = new FaceInfoCache();
        faceViewCache = new FaceViewCache();
        tvNumber = mCallView.getCallActivity().findViewById(R.id.tv_number);
        iv_host_num = mCallView.getCallActivity().findViewById(R.id.iv_host_num);
    }
        ...
        NemoSDK.getInstance().setNemoSDKListener(new SimpleNemoSDkListener() {
            ...
            public void onRosterChange(RosterWrapper roster) {
                if (roster != null) {
                    ...
                    Log.e("参会人数：" + (roster.getRosters().size() + 1));
                    mCallView.getCallActivity().runOnUiThread(new Runnable() {
                        @Override
                        public void run() {
                            tvNumber.setText((roster.getRosters().size() + 1) + "人在线");
                            tvNumber.setVisibility(View.GONE);
                            if ((roster.getRosters().size() + 1) < 1) {
                                iv_host_num.setText("1");
                            } else {
                                iv_host_num.setText((roster.getRosters().size() + 1) + "");
                            }
                        }
                    });
                }
            ...
            @Override
            public void onConfMgmtStateChanged(int callIndex, String operation, boolean isMuteIsDisabled) {
                L.i(TAG, "onConfMgmtStateChanged: " + operation + " isMuteIsDisabled: " + isMuteIsDisabled);
                Observable.just(0).observeOn(AndroidSchedulers.mainThread())
                        .subscribe(integer -> mCallView.showConfMgmtStateChanged(operation, isMuteIsDisabled));
            }
            ...
            public void onKickOut(int code, int reason) {
                L.i(TAG, "onKickOut called code="+code+"reason= "+reason);
                Observable.just(0).observeOn(AndroidSchedulers.mainThread())
