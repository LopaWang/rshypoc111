...
public class BackgroundCallService extends Service {
    ...
    public Notification getInCallNotification() {
        Intent intent = new Intent(PocApp.getInstance(), XyCallActivity.class);
        PendingIntent pendingIntent = PendingIntent.getActivity(PocApp.getInstance(),
                UUID.randomUUID().hashCode(), intent, PendingIntent.FLAG_UPDATE_CURRENT);
        NotificationCompat.Builder builder = new NotificationCompat.Builder(PocApp.getInstance(), getInCallChannelId())
                .setSmallIcon(R.mipmap.ic_launcher)
                .setDefaults(NotificationCompat.FLAG_ONGOING_EVENT)
                .setSound(null)
                .setVibrate(new long[]{0})
                .setContentTitle(PocApp.getInstance().getString(R.string.app_name))
                .setContentText("正在运行")
